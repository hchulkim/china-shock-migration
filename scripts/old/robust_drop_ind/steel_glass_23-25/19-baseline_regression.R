...
imp <- read_rds("data/final/shock/imp_cz_drop2325.rds")
exp <- read_rds("data/final/shock/exp_cz_drop2325.rds")

imp1999_iv <-read_rds("data/final/shock/imp1999_iv_cz_drop2325.rds")
exp1999_iv <-read_rds("data/final/shock/exp1999_iv_cz_drop2325.rds")
...
tb_all <- tb_all %>% select(-c("imp_shock_o", "exp_shock_o", "imp_shock_d", "exp_shock_d", "imp_shock1999_iv_o", "exp_shock1999_iv_o", "imp_shock1999_iv_d", "exp_shock1999_iv_d"))

tb_all <- tb_all %>% left_join(imp, by=c("cz_o"="cz")) %>% rename(imp_shock_o=imp_shock)
tb_all <- tb_all %>% left_join(imp, by=c("cz_d"="cz")) %>% rename(imp_shock_d=imp_shock)

tb_all <- tb_all %>% left_join(exp, by=c("cz_o"="cz")) %>% rename(exp_shock_o=exp_shock)
tb_all <- tb_all %>% left_join(exp, by=c("cz_d"="cz")) %>% rename(exp_shock_d=exp_shock)


tb_all <- tb_all %>% left_join(imp1999_iv, by=c("cz_o"="cz")) %>% rename(imp_shock1999_iv_o=imp_shock)
tb_all <- tb_all %>% left_join(imp1999_iv, by=c("cz_d"="cz")) %>% rename(imp_shock1999_iv_d=imp_shock)

tb_all <- tb_all %>% left_join(exp1999_iv, by=c("cz_o"="cz")) %>% rename(exp_shock1999_iv_o=exp_shock)
tb_all <- tb_all %>% left_join(exp1999_iv, by=c("cz_d"="cz")) %>% rename(exp_shock1999_iv_d=exp_shock)
...
# add two period shock

imp2010 <- read_rds("data/final/shock/imp2010_cz_drop2325.rds") %>% mutate(year=1)
imp2019 <- read_rds("data/final/shock/imp2019_cz_drop2325.rds") %>% mutate(year=2)
exp2010 <- read_rds("data/final/shock/exp2010_cz_drop2325.rds") %>% mutate(year=1)
exp2019 <- read_rds("data/final/shock/exp2019_cz_drop2325.rds") %>% mutate(year=2)

imp1999_1_iv <-read_rds("data/final/shock/imp1999_1_iv_cz_drop2325.rds") %>% mutate(year=1)
exp1999_1_iv <-read_rds("data/final/shock/exp1999_1_iv_cz_drop2325.rds") %>% mutate(year=1)

imp1999_2_iv <-read_rds("data/final/shock/imp1999_2_iv_cz_drop2325.rds") %>% mutate(year=2)
exp1999_2_iv <-read_rds("data/final/shock/exp1999_2_iv_cz_drop2325.rds") %>% mutate(year=2)


imp <- imp2010 %>% bind_rows(imp2019)
exp <- exp2010 %>% bind_rows(exp2019)

imp1999_iv <- imp1999_1_iv %>% bind_rows(imp1999_2_iv)
exp1999_iv <- exp1999_1_iv %>% bind_rows(exp1999_2_iv)
...
tb_two <- tb_two %>% select(-c("imp_shock_o", "exp_shock_o", "imp_shock_d", "exp_shock_d", "imp_shock1999_iv_o", "exp_shock1999_iv_o", "imp_shock1999_iv_d", "exp_shock1999_iv_d"))

tb_two <- tb_two %>% left_join(imp, by=c("cz_o"="cz", "year")) %>% rename(imp_shock_o=imp_shock)
tb_two <- tb_two %>% left_join(imp, by=c("cz_d"="cz", "year")) %>% rename(imp_shock_d=imp_shock)

tb_two <- tb_two %>% left_join(exp, by=c("cz_o"="cz", "year")) %>% rename(exp_shock_o=exp_shock)
tb_two <- tb_two %>% left_join(exp, by=c("cz_d"="cz", "year")) %>% rename(exp_shock_d=exp_shock)


tb_two <- tb_two %>% left_join(imp1999_iv, by=c("cz_o"="cz", "year")) %>% rename(imp_shock1999_iv_o=imp_shock)
tb_two <- tb_two %>% left_join(imp1999_iv, by=c("cz_d"="cz", "year")) %>% rename(imp_shock1999_iv_d=imp_shock)

tb_two <- tb_two %>% left_join(exp1999_iv, by=c("cz_o"="cz", "year")) %>% rename(exp_shock1999_iv_o=exp_shock)
tb_two <- tb_two %>% left_join(exp1999_iv, by=c("cz_d"="cz", "year")) %>% rename(exp_shock1999_iv_d=exp_shock)
