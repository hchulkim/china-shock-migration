...
# this code makes hetereogenity data for migration from 2001-2019 and 2020.
...
colnames(migration2001) = c("전입행정_시도"
                            , "전입행정_시군구", "전입행정_동읍면", "전입년", "전입월", "전입일", "전출행정_시도", "전출행정_시군구", "전출행정_동읍면", "reason", "relation", "age_h", "gen_h", "household", "이동_총인구", "이동_남인구", "이동_여인구")
...
colnames(migration2015) = c("전입행정구역_시도코드"
                            , "전입행정구역_시군구코드", "전입행정구역_읍면동코드", "전입연도", "전입월", "전입일", "전출행정구역_시도코드", "전출행정구역_시군구코드", "전출행정구역_읍면동코드", "reason", "relation", "age_h", "gen_h", "household", "이동_총인구수", "이동_남자인구수", "이동_여자인구수")
...
migration2001_2014 <- migration2001_2014 %>% mutate(`이동_총인구`=as.numeric(`이동_총인구`), `이동_여인구`=as.numeric(`이동_여인구`), `이동_남인구`=as.numeric(`이동_남인구`))

migration2001_2014 <- migration2001_2014 %>% mutate(reason=as.numeric(reason), relation=as.numeric(relation), age_h=as.numeric(age_h), gen_h=as.numeric(gen_h), household=as.numeric(household))


migration2001_2014 <- migration2001_2014 %>%
  bind_rows(migration2010 %>% mutate(reason=as.numeric(reason), relation=as.numeric(relation), age_h=as.numeric(age_h), gen_h=as.numeric(gen_h), household=as.numeric(household)))

rm(migration2010)

migration2001_2014 <- migration2001_2014 %>%
  bind_rows(migration2011 %>% mutate(reason=as.numeric(reason), relation=as.numeric(relation), age_h=as.numeric(age_h), gen_h=as.numeric(gen_h), household=as.numeric(household)))

rm(migration2011)

migration2001_2014 <- migration2001_2014 %>%
  bind_rows(migration2012 %>% mutate(reason=as.numeric(reason), relation=as.numeric(relation), age_h=as.numeric(age_h), gen_h=as.numeric(gen_h), household=as.numeric(household)))

rm(migration2012)

migration2001_2014 <- migration2001_2014 %>%
  bind_rows(migration2013 %>% mutate(reason=as.numeric(reason), relation=as.numeric(relation), age_h=as.numeric(age_h), gen_h=as.numeric(gen_h), household=as.numeric(household)))

rm(migration2013)

migration2001_2014 <- migration2001_2014 %>%
  bind_rows(migration2014 %>% mutate(reason=as.numeric(reason), relation=as.numeric(relation), age_h=as.numeric(age_h), gen_h=as.numeric(gen_h), household=as.numeric(household)))

rm(migration2014)


migration2001_2014 %>% write_rds("data/migration2001_2014_hetero.rds")
...
migration2015_2019 %>% write_rds("data/migration2015_2019_hetero.rds")
...
migration1996_2000 %>% write_rds("data/migration1996_2000.rds")
migration2001_2014 %>% write_rds("data/migration/migration2001_2014_hetero.rds")
migration2015_2019 %>% write_rds("data/migration/migration2015_2019_hetero.rds")
...
migration2001_2014 <- read_rds("data/migration/migration2001_2014_hetero.rds")
migration2015_2019 <- read_rds("data/migration/migration2015_2019_hetero.rds")
...
migration2001_2014 %>% write_rds("data/migration/migration2001_2014_hetero_refine.rds")
migration2015_2019 %>% write_rds("data/migration/migration2015_2019_hetero_refine.rds")



migration2001_2014 %>% haven::write_dta("data/migration/migration2001_2014_hetero_refine.dta")
migration2015_2019 %>% haven::write_dta("data/migration/migration2015_2019_hetero_refine.dta")
migration1996_2000 %>% haven::write_dta("data/migration/migration1996_2000_refine.dta")



migration1996_2000 %>% write_rds("data/final/migration/migration1996_2000_refine.rds")
migration2001_2014 %>% write_rds("data/final/migration/migration2001_2014_hetero_refine.rds")
migration2015_2019 %>% write_rds("data/final/migration/migration2015_2019_hetero_refine.rds")



migration2001_2014 %>% haven::write_dta("data/final/migration/migration2001_2014_hetero_refine.dta")
migration2015_2019 %>% haven::write_dta("data/final/migration/migration2015_2019_hetero_refine.dta")
migration1996_2000 %>% haven::write_dta("data/final/migration/migration1996_2000_refine.dta")


migration1996_2000 <- migration1996_2000 %>% mutate(migrate=`연령별남자(합계)`+`연령별여자(합계)`) %>% select(year, iso_o, iso_d, migrate, city_label)
...
migration2020 %>% write_rds("data/final/migration/migration2020_hetero_refine.rds")

migration2020 %>% haven::write_dta("data/final/migration/migration2020_hetero_refine.dta")
