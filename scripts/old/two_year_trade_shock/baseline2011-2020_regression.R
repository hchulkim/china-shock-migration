...
out_dest <- feols(log(migrate) ~ log(pop2001_d) + log(pop2001_o) + log(distance) | 
                    shock  ~ shock_iv, 
...
out_org <- feols(log(migrate) ~ log(pop2001_d) + log(pop2001_o) + log(distance) | 
                   shock  ~ shock_iv, 
                 weights = tb_all$pop2001_o, cluster=tb_all$iso_o, data=tb_all %>% 
                   mutate(shock=(exp_shock_o - imp_shock_o)/1000, shock_iv=(exp_shock1999_iv_o - imp_shock1999_iv_o)/1000))

out_all <- feols(log(migrate) ~ log(pop2001_d) + log(pop2001_o) + log(distance) | 
